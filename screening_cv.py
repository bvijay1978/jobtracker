"""Generate an ATS-safe "screening" CV: plain, single-column, keyword-aligned.
...
produced by the agent layer, which reads the job description and maps its terms
to the candidate's genuine experience; this module renders that into a
...
def generate_screening_cv(role: dict, screening: dict, profile_path=None, out_dir=None) -> Path:
    """Render an ATS-safe screening CV from an agent-produced ``screening`` payload.
    ...
    path = out_dir / f"{prof.get('name', 'CV')} - Screening - {_safe(target)}.docx"
    ...
    _run(doc.add_paragraph(), prof.get("name", ""), size=16, bold=True)
    _run(doc.add_paragraph(), target, size=12)
    _run(doc.add_paragraph(), prof.get("contact", ""), size=10)
    ...
    for job in screening.get("experience") or prof.get("jobs", []):
    ...
    if prof.get("certifications"):
        _run(doc.add_paragraph(), prof["certifications"])
    if prof.get("education"):
        _run(doc.add_paragraph(), prof["education"])
